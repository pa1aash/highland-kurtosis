...
9. **Phase 3 stacked-layer N-scaling with Geant4 (2026-03-23, INVALIDATED):** Figure `paper/figure_6_n_scaling_with_geant4.{png,pdf}` and `results/phase3_stacked_results.json` were generated from data with **wrong cellSize** (1.78mm → ~40% effective infill, not 20%). All 5 macros in `macros/generated/phase3_stacked/` have been corrected to cellSize=3.79mm. **Data must be regenerated** by rerunning `scripts/run_stacked_layers.sh`.
   - Bug: cellSize=1.78mm is the 40% infill value (from CELL_SIZES dict in generate_macros.py); 20% requires 3.79mm
   - C++ code (`ConstructStackedRectilinearLattice`) is correct: random offsets work, walls confined to layer z-slices, path-length variance scales as ~1/N
   - Quick test with corrected cellSize: N=1 κ≈58.8, N=4 κ≈36.6 (10k events, high stat uncertainty)
   - κ does decrease with N but is dominated by MCS baseline κ_M/f ≈ 23.5 at 20% infill
