...
- **Content:** 8 systematic contributions + statistical + totals for rect 40% 4 GeV
- **Columns:** Source, δκ, Method
- **Data source:** `analysis/systematic_uncertainty.py` using thin-wall results, cut-variation sweep, model comparison solid control ROOT files
- **Key values:** Angle cut (8.52, 7–15σ range), Highland log (3.05), fiducial (3.11), G4 model (2.32, from solid controls), thin-wall κ_M (1.32), stat SE (1.35). Total systematic = 9.95.
- **Notes:** Angle cut restricted to 7–15σ (excluding 5σ tail truncation and 20σ nuclear outliers). Geant4 model comparison uses solid controls because rect40 model files have wrong effective infill (~20% instead of 40%).
...
7. **Systematic uncertainty table (2026-03-23):** New file `paper/table_systematics.tex` generated by `analysis/systematic_uncertainty.py`. LaTeX table with 8 systematic sources for rect 40% 4 GeV. Not yet \input in main.tex. Key findings:
   - Angle cut threshold is the dominant systematic (δκ = 8.52), from 7–15σ range (5σ/20σ excluded as non-physical extremes)
   - Highland log correction (δκ = 3.05): analytic bias from nonlinear log term at ℓ/X₀ ≪ 1
   - Fiducial region (δκ = 3.11): edge effects at |x| > 5 mm
   - Geant4 MCS model (δκ = 2.32): from solid control files (opt3 κ_M=2.15 vs opt4 κ_M=4.47)
   - rect40 model comparison files have wrong geometry (~20% effective infill, mean pla_path ~1.96 mm); solid controls used instead
   - Total systematic = 9.95, total combined = 10.04
